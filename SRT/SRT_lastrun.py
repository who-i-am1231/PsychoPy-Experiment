...
expName = 'SRT'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\LIANG VALENTINO\\Desktop\\北京化工大学-2024-生物工程-梁振洲-PsychoPy实验包\\SRT\\SRT_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    WT = visual.TextStim(win=win, name='WT',
        text='欢迎您来带著名的SRT实验，\n接下来我们会给您展示一个会移动的红色圆点，\n其只会出现在从左到右的四个位置，依次对应着d,f,j,k\n需要您尽可能快的判断其位置，并按下对应的按钮，现在按下任意键，\n我们会为您呈现四个位置与对应的按钮\n',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=5.0, ori=0.0, 
    ...
    text = visual.TextStim(win=win, name='text',
        text='',
        font='Arial',
        pos=(0, -5), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    EX = visual.ShapeStim(
    ...
    text_2 = visual.TextStim(win=win, name='text_2',
        text='实验将在5s后正式开始，请将手指放在 d f j k 上做好准备',
        font='Arial',
    ...
    text_3 = visual.TextStim(win=win, name='text_3',
        text='感谢您参加我们的实验，本次实验将在10s后自动退出',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    while continueRoutine:
        ...
        # if WT is starting this frame...
        if WT.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            WT.frameNStart = frameN  # exact frame index
            WT.tStart = t  # local t and not account for scr refresh
            WT.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(WT, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'WT.started')
            # update status
            WT.status = STARTED
            WT.setAutoDraw(True)
        
        # if WT is active this frame...
        if WT.status == STARTED:
            # update params
            pass
    ...
    exa = data.TrialHandler2(
        name='exa',
        nReps=1.0, 
        method='sequential', 
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('EXAMPLE.xlsx'), 
        seed=None, 
    )
    thisExp.addLoop(exa)  # add the loop to the experiment
    thisExa = exa.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisExa.rgb)
    if thisExa != None:
        for paramName in thisExa:
            globals()[paramName] = thisExa[paramName]
    ...
    for thisExa in exa:
        exa.status = STARTED
        if hasattr(thisExa, 'status'):
            thisExa.status = STARTED
        currentLoop = exa
        ...
        if 'corr_ans' in globals():
            corr_ans = globals()['corr_ans']
        ...
        while continueRoutine:
            ...
            if hasattr(thisExa, 'status') and thisExa.status == STOPPING:
                continueRoutine = False
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            if EX.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'EX.started')
                # update status
                EX.status = STARTED
                EX.setAutoDraw(True)
            
            # if EX is active this frame...
            ...
            if key_resp_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # allowed keys looks like a variable named `corr_ans`
                if not type(corr_ans) in [list, tuple, np.ndarray]:
                    if not isinstance(corr_ans, str):
                        corr_ans = str(corr_ans)
                    elif not ',' in corr_ans:
                        corr_ans = (corr_ans,)
                    else:
                        corr_ans = eval(corr_ans)
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(key_resp_2.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(key_resp_2.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if key_resp_2.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_2.getKeys(keyList=list(corr_ans), ignoreKeys=["escape"], waitRelease=False)
        ...
        routineTimer.reset()
        # mark thisExa as finished
        if hasattr(thisExa, 'status'):
            thisExa.status = FINISHED
        # if awaiting a pause, pause now
        if exa.status == PAUSED:
    ...
    if begin.maxDurationReached:
        routineTimer.addTime(-begin.maxDuration)
    elif begin.forceEnded:
        routineTimer.reset()
    else:
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('the_condition.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            if key_resp_3.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_3.getKeys(keyList=['d','f','j','k'], ignoreKeys=["escape"], waitRelease=False)
